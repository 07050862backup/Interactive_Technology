# Change blue color to yellow color
import cv2
import numpy as np

cap = cv2.VideoCapture(0)
ret, frame = cap.read()
rows, columns, channels = frame.shape
mask01 = np.zeros((rows, columns), np.uint8)
yellow = np.full(frame.shape, (0, 255,255), dtype=np.uint8)
    
# define range of blue color in HSV
lower_Hue = np.array([81, 50, 50], dtype=np.uint8)
upper_Hue = np.array([129,255,255], dtype=np.uint8)

luck_ori = cv2.imread('luck_ori.jpg')
luck_ori = cv2.resize(luck_ori,(640,480))
luck = cv2.imread('luck.jpg')
luck = cv2.resize(luck,(640,480))
while(1):

    # Take each frame
    ret, frame = cap.read()
    frame = cv2.flip(frame, 1)
    yellowedFrame= cv2.addWeighted(luck, 0, luck_ori,1, 0)

    # Convert BGR to HSV
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # Threshold the HSV image to get only blue colors
    mask = cv2.inRange(hsv, lower_Hue, upper_Hue)
    frame = luck
    # apply a series of erosions and dilations to the mask
    # using an elliptical kernel
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
    mask = cv2.erode(mask, kernel, iterations = 2)
    mask = cv2.dilate(mask, kernel, iterations = 2)
    mask01 = cv2.bitwise_or(mask01, mask)
    mask02 = cv2.bitwise_not(mask01)
    # Bitwise-AND mask and original image
    fg = cv2.bitwise_and(frame,frame, mask= mask01)
    bg = cv2.bitwise_and(yellowedFrame, yellowedFrame, mask= mask02)
    final = cv2.add(fg, bg)
    cv2.imshow('frame', frame)
    cv2.imshow('mask', mask)
    cv2.imshow('mask01',mask01)
    cv2.imshow('yellowedFrame',yellowedFrame)
    cv2.imshow('final',final)

    k = cv2.waitKey(5) & 0xFF
    if k == 27:
        break

cv2.destroyAllWindows()